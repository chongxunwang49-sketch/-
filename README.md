...
**v4.0 UI(v4.0 全面进化)**:Neumorphism + Glassmorphism 玻璃拟态、霓虹蓝紫渐变、弹性缓动 `cubic-bezier(0.34,1.56,0.64,1)`、点击涟漪、页面入场、Tab 横向滑动。登录页双栏(左 SVG 品牌轮播 + 右玻璃登录卡)、顶部状态栏(实时时钟/数据源灯/Ctrl+K 搜索/头像下拉)、驾驶舱指数 marquee + 撒花彩蛋、深度研究计速器报价 + 动态 MA 滑块 + Agent 拖拽拓扑图(ECharts)、股小智打字机 + 画中画进度卡、个人中心环形进度、后台 CPU/内存液位图(psutil)、思维链鱼缸 + 赛博朋克模式彩蛋。导航:📊 驾驶舱 | 📈 深度研究 | 🤖 股小智 | ⭐ 星标自选 | 🗂 历史轨迹 | ⚙️ 个人中心。
